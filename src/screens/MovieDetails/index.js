        ...
        {isSuccess && (
          ...
          >
            <div className="col-span-3 sm:col-span-3 xs:col-span-3 md:col-span-1 lg:col-span-1 grid place-content-center p-5">
              ...
              <img
                src={`${getFullImageUrl(data.data?.poster_path)}`}
            ...
            <div className="col-span-3 sm:col-span-3 xs:col-span-3 md:col-span-2 lg:col-span-2 flex flex-col bg-white m-5 bg-opacity-80 rounded p-5">
                ...
                {data?.data?.homepage && (
                  <span>
                    <BsLink
                      ...
                      onClick={() => window.open(data.data?.homepage)}
                    />
                  </span>
                )}
              </div>
              ...
              <div className="text-md pb-2">{data.data.overview}</div>
              {!!data.data?.vote_average && !!data.data?.vote_count && (
                <div className="text-md pt-2">
                  <div>
                    {`Rated: `}
                    <strong>{data.data?.vote_average}</strong>
                    {` from `}
                    <strong>
              ...
              {!!data.data?.production_countries && (
                <div className="text-md pt-2">
                  <div>
                    ...
                    {data.data?.production_countries?.map((country, key) => (
                      <span
                        className="inline-block bg-gray-200 rounded-full px-3 py-1 text-xs font-semibold text-gray-700 mr-2"
                        key={key}
                      >
              ...
              {!!data.data?.spoken_languages && (
                <div className="text-md pt-2">
                  <div>
                    Spoken languages:{` `}
                    {data.data?.spoken_languages?.map((language, key) => (
                      <span
                        className="inline-block bg-gray-200 rounded-full px-3 py-1 text-xs font-semibold text-gray-700 mr-2"
                        key={key}
                      >
              ...
              {!!data.data?.budget && (
                <div className="text-md pt-2">
                  <div>
                    Budget: <strong>{data.data?.budget}</strong>
                  </div>
                </div>
              )}
              {!!data.data?.original_language && (
                <div className="text-md pt-2">
                  <div>
                    Original Language:{" "}
                    <strong>{data.data?.original_language}</strong>
                  </div>
                </div>
              )}
              {!!data.data?.popularity && (
                <div className="text-md pt-2">
                  <div>
                    Popularity: <strong>{data.data?.popularity}</strong>
                  </div>
                </div>
              )}
              {!!data.data?.revenue && (
                <div className="text-md pt-2">
                  <div>
                    Revenue: <strong>{data.data?.revenue}</strong>
                  </div>
                </div>
              )}
            </div>
